...
#[derive(Clone, Debug)]
struct ResolvedItemInfo<'a> {
    original_item: &'a Item,
    effective_name: Option<String>, // Name in the current scope (alias or original)
    reexport_source_canonical_path: Option<String>, // Canonical path of the original item, if re-exported
}

fn get_item_kind_string(item_enum: &ItemEnum) -> &str {
    match item_enum {
        ...
        ItemEnum::Use(_) => "Use Statement", // Should primarily be handled by format_use_statement_for_listing
...
fn process_items(output: &mut String, item_ids: &[Id], data: &Crate, level: usize) {
        ...
        if let Some(item) = data.index.get(&id) {
            match &item.inner {
                ItemEnum::Use(use_item) => {
                    ...
                    // Phase 1: Process named re-exports for inlining
                    if !use_item.is_glob {
                        if let Some(target_id) = use_item.id {
                            if let Some(target_item) = data.index.get(&target_id) {
                                // Check if target is struct or enum as per Phase 1 focus,
                                // but generally, we can try to inline any named re-export.
                                // For Phase 1, let's stick to the plan's focus on structs/enums.
                                match &target_item.inner {
                                    ...
                                    _ => {
                                        // Other re-exported types will just be listed in the "Re-exports" section for now.
                                    }
                                }
                            }
                        }
                    }
                    // Glob re-exports are just listed in the "Re-exports" section for Phase 1.
                }
                _ => {
                    // Direct items
                    all_resolved_items.push(ResolvedItemInfo {
    ...
    // Note: 'reexports' (use_item_ids_for_reexport_section) are handled separately.
    ...
    for resolved_info in &all_resolved_items {
        match &resolved_info.original_item.inner {
            ...
            ItemEnum::Use(_) => {} // These are handled by use_item_ids_for_reexport_section
...
fn format_use_statement_for_listing(output: &mut String, item: &Item, _data: &Crate, level: usize) {
    let heading_level = std::cmp::min(level, 6); // Use level+1 from process_items for consistency
    ...
    if let ItemEnum::Use(use_item) = &item.inner {
        ...
        if use_item.is_glob {
            ...
            // Use item.name for the alias, use_item.name is the same if not aliased
            let display_name = item.name.as_ref().unwrap_or(&use_item.name);
            if display_name != source_name_segment && item.name.is_some() { // Aliased
                 output.push_str(&format!("{} `use {} as {}`\n\n", heading, use_item.source, display_name));
            } else { // Not aliased or item.name is not the alias part
                 output.push_str(&format!("{} `use {}`\n\n", heading, use_item.source));
            }
        }

        // Add documentation for the use statement itself, if any
        if let Some(docs) = &item.docs {
            output.push_str(&format!("{}\n\n", docs));
        }

        // Add the 'use ...;' line in a code block
        ...
        } else if let Some(name_attr) = &item.name { // item.name here is the alias if `as Alias` was used
            // Check if it's a renaming alias
...
fn process_item(output: &mut String, resolved_info: &ResolvedItemInfo, data: &Crate, level: usize) {
    ...
    if let Some(display_name) = display_name_opt {
        if let Some(canonical_path) = &resolved_info.reexport_source_canonical_path {
            // This is a re-exported item.
            // Format: "Re-exported Kind `DisplayName` (from `CanonicalPath`)"
            // If DisplayName is an alias (i.e., different from original item name),
            // it's already captured in display_name.
            output.push_str(&format!(
                "{} Re-exported {} `{}` (from `{}`)\n\n",
            ...
            // Special handling for Module level to ensure it's ##
        ...
        // Handle nameless items like Impl
        match &item.inner {
            ItemEnum::Impl(impl_details) => {
                if let Some(trait_) = &impl_details.trait_ {
                    output.push_str(&format!(
                        ...
                        heading_prefix,
                        trait_.path,
                        format_type(&impl_details.for_, data)
                    ));
                } else {
                    output.push_str(&format!(
                        "{} Implementation for `{}`\n\n",
            ...
            _ => {
                // For other truly unnamed items, or items where name is not applicable for heading
    ...
    // Add code block with item signature (using the original item for signature)
    output.push_str("```rust\n");
    format_item_signature(output, item, data); // Pass original 'item'
    output.push_str("\n```\n\n");

    // Process additional details based on item kind (using original item)
    // Phase 1 focuses on basic rendering; details functions are not changed yet.
    match &item.inner {
...
fn process_impl_details(output: &mut String, impl_: &Impl, data: &Crate, level: usize) {
    ...
    if !impl_.items.is_empty() {
        ...
        if !assoc_consts.is_empty() {
            ...
            for &const_id in &assoc_consts {
                if let Some(assoc_item_ref) = data.index.get(&const_id) {
                    let resolved_assoc_info = ResolvedItemInfo {
                        original_item: assoc_item_ref,
                        effective_name: assoc_item_ref.name.clone(),
                        reexport_source_canonical_path: None,
        ...
        if !methods.is_empty() {
            ...
            for &method_id in &methods {
                if let Some(assoc_item_ref) = data.index.get(&method_id) {
                    let resolved_assoc_info = ResolvedItemInfo {
                        original_item: assoc_item_ref,
                        effective_name: assoc_item_ref.name.clone(),
                        reexport_source_canonical_path: None,
